...
## Outbound Copy Typography

This section applies to every complete piece of copy that Ranjodh is expected to send, post, submit, paste, or otherwise use as an outbound communication or response.

Covered artifacts include, but are not limited to:

- external email and email replies
- internal Slack or Teams messages
- SMS, WhatsApp-style messages, and direct messages
- LinkedIn messages, replies, posts, or comments
- support-ticket text intended for submission
- customer-facing or internal responses
- calendar-response wording
- any other complete message or sendable copy generated by a workflow

### No em dash

Never include the Unicode em dash character `—` (U+2014) anywhere inside an outbound artifact.

This rule applies to:

- email subjects and bodies
- greetings and closings
- Slack, Teams, SMS, LinkedIn, or other message text
- bullets and numbered items inside sendable copy
- rewritten or quoted source text included in the outbound artifact
- support-ticket Subject or Description text
- workflow-generated message templates

Before rendering the final outbound artifact, perform a character-level check for U+2014. If any em dash remains, rewrite the sentence using natural punctuation such as a comma, colon, semicolon, period, parentheses, or a simple hyphen when grammatically appropriate.

Do not replace an em dash with an en dash merely to preserve the same rhetorical punctuation. En dashes may still be used for genuine numeric/date ranges when appropriate, and normal hyphens may still be used in compound words.

If exact source evidence contains an em dash but exact verbatim preservation is not required, normalize it before placing it inside the outbound artifact. If an exact literal technical string must remain verbatim, keep that exact evidence outside the sendable draft when possible rather than violating this outbound-copy rule.

This rule overrides conflicting examples, templates, tone guidance, or source punctuation in supporting skills.
